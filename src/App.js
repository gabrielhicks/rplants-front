...
const PlantsContainer = lazy(() =>
    import('./components/PlantsContainer/PlantsContainer')
);
...
class App extends React.Component {
    ...
    componentDidMount() {
        ...
        if (token) {
            ...
            fetch(
                'https://rplants-backend.herokuapp.com/api/v1/profile',
                options
            )
                .then((res) => res.json())
    ...
    deleteHandler = (id, size, quantity) => {
        ...
        const priceToSubtract =
            parseFloat(item.price[item.size]) * parseFloat(item.quantity);
        const newTotal =
            Math.round(
                (parseFloat(this.state.cartTotal) -
                    priceToSubtract +
                    Number.EPSILON) *
                    100
            ) / 100;
    ...
    addToCart = (item, size, quantity) => {
        ...
        const newTotal =
            Math.round(
                (this.state.cartTotal +
                    parseFloat(item.price[size]) * parseFloat(quantity) +
                    Number.EPSILON) *
                    100
            ) / 100;
        this.setState((prev) => ({
        ...
        let index = this.state.cart.findIndex(
            (cartItem) => cartItem.size === size && cartItem.id === item.id
        );
        if (index === -1) {
                ...
                <Suspense fallback={<div>Loading...</div>}>
                    <Switch>
                        ...
                        <Route
                            ...
                            render={() => (
                                <NewPlant
                                    submitHandler={this.addPlantHandler}
                                />
                            )}
                        />
                        <Route
                            ...
                            render={() => (
                                <Checkout
                                    redirectToSuccess={this.redirectToSuccess}
                                    clearGuestUser={this.clearGuestUser}
                                    cart={this.state.cart}
                                    total={this.state.cartTotal}
                                    user={this.state.user}
                                />
                        ...
                        <Route
                            ...
                            render={() => (
                                <Success
                                    order_number={
                                        this.state.completedOrderNumber
                                    }
                                    redirectToWelcome={this.redirectToWelcome}
                                />
                            )}
                        />
                        <Route
                            path='/'
                            render={() => (
                                <Welcome plants={this.state.plants} />
                            )}
                        />
                    </Switch>
                </Suspense>
